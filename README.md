...
- **Messages** - the conversation list and bulk-action screen.
  - Shows who a text claims to be **on behalf of** (e.g. "Paid for
    by...", "on behalf of...") right in the list when the message states
    it, with the raw phone number underneath - not just a bare number,
    the same attribution logic the Log/Reports tabs use.
...
- **Log** - a running, searchable record of every incoming text the app
  has seen: when it arrived, who it was from, and - when the message
  itself says so (e.g. "Paid for by...", "on behalf of...", "Authorized
  by...") - who it claims to be sent on behalf of. Search by sender,
  attribution, or message text, and filter by 7/30/90 days, 1 year, or
  all time. Whatever bulk action you take on a conversation (reply,
  block, report) is recorded against its log entry too.
  Conversations with anything still unread show an **UNREAD** tag; that
  clears once you apply a bulk action to them.
- **Reports** - the same log, broken down into day / week / month /
  quarter (3 months) / half-year (6 months) / year totals, each showing
  how many texts came in and from which senders (grouped by the "on
  behalf of" name when the message states one, otherwise by phone
  number).
