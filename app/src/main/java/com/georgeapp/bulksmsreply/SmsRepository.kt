...
object SmsRepository {
    ...
    /**
     * Reads every individual inbox message (not collapsed by sender) so
     * the message log can record each one. Used for the log/reports
     * feature - separate from [loadConversations], which only needs the
     * latest message per sender for the main list screen.
     */
    fun loadAllRawMessages(context: Context): List<RawSmsMessage> {
        val projection = arrayOf(
            Telephony.Sms.ADDRESS,
            Telephony.Sms.BODY,
            Telephony.Sms.DATE
        ...
        context.contentResolver.query(
            ...
            while (cursor.moveToNext()) {
                ...
                val body = cursor.getString(bodyIndex) ?: ""
                val date = cursor.getLong(dateIndex)
                messages.add(RawSmsMessage(address = rawAddress, body = body, dateMillis = date))
            }
        }
